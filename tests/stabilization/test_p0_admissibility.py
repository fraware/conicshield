...
@pytest.mark.xfail(
    strict=True,
    reason="CS-SOLVER-002: native path releases solution.x without finite/residual admissibility checks",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="CS-SOLVER-002: CVXPY path accepts any finite x.value without inaccurate/timeout release policy",
)
def test_cvxpy_path_must_enforce_declared_status_release_policy() -> None:
    """Document gap: existence of x.value is not enough; inaccurate/limited statuses must be policy-gated."""
    # A dedicated verifier module is required before release (S1+). Until then this import fails the xfail.
    from conicshield.specs import admissibility  # type: ignore[attr-defined]
