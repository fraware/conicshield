...
**Fixture source:** `tests/fixtures/parity_reference/` promoted from [`wsl-real-20260409-132450`](../benchmarks/published_runs/wsl-real-20260409-132450/) (S2 reference). Record: [`REGENERATION_NOTE.md`](../tests/fixtures/parity_reference/REGENERATION_NOTE.md). Flagship parity gold is unchanged when `current_run_id` is `host-realistic-20260525`.

**Index:** [`benchmarks/PUBLISHED_RUN_INDEX.json`](../benchmarks/PUBLISHED_RUN_INDEX.json) — refresh with `python scripts/refresh_published_run_index.py` (`--check` in CI).

**See also:** [BENCHMARK_GOVERNANCE.md](BENCHMARK_GOVERNANCE.md), [MAINTAINER_RUNBOOK.md](MAINTAINER_RUNBOOK.md).
