...
1. **Upstream and pin:** Align `inter-sim-rl` semantics (action-conditioned transitions, `get_shield_context`) on your fork or upstream. Update `third_party/inter-sim-rl/REVISION` (`sha=`, `commit_url=`, `recorded_utc`) when the checkout changes. With a full git checkout, [`tests/environment/test_third_party_pins.py`](../tests/environment/test_third_party_pins.py) must pass in CI.
