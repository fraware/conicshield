...
**Commands:** `python scripts/performance_benchmark.py` (single-scenario default; emits **`native_microbatch`** vs **`native_compiled_real_batch`** using **`--batch-size`** default 4). Use **`--sweep`** for `action_dim × conditioning × scenario` grids (default dims `4,8`; override with `--shield-action-dims`). Add **`--batch-sizes`** in sweep mode for multiple batch widths. Summarize speedup with **`python scripts/batch_solve_report.py`**. **`--sweep-auto-tune`** compares `NativeMoreauCompiledOptions.auto_tune`. Programmatic batching: `Backend.NATIVE_MOREAU_BATCH` and `create_batch_projector()` → `NativeMoreauCompiledBatchProjector`.
