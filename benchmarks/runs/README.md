...
6. Copy the validated bundle to [`benchmarks/published_runs/<run_id>/`](../published_runs/README.md) before merge if your workflow keeps a canonical published copy (recommended once `CURRENT.json` will reference this `run_id`). When you change files under `benchmarks/published_runs/`, refresh and commit [`benchmarks/PUBLISHED_RUN_INDEX.json`](../PUBLISHED_RUN_INDEX.json) (`python scripts/refresh_published_run_index.py`; use `--check` in CI-style verification).
